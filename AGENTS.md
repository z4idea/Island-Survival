...
- Last refreshed: 2026-06-12（refresh pass：洞穴生态（吸血蝙蝠/妖狐/宝藏洞穴）与状态系统落地后全文核对，存档版本/状态接线/近战免疫已 grep 源码验证）
...
| [src/entities/player.ts](src/entities/player.ts) | 玩家：移动/翻滚(无敌帧)/武器库存(`weapons[]`+`weaponIdx`,1~8 键)/`weaponDmg`(基础×武器等级×篝火强化)/进食(解毒)/**状态效果**（`statuses: Statuses`——`applyPoison`/`applyCharm`(WASD 反向)/`drainBlood`(蝙蝠吸血,无视无敌帧)/溺水维持态/`clearStatuses`,图标同步在 update 末尾按 `statusKey` 变更触发）/乘船(`sailing`,自动上下船+碰撞组切换)/皮肤粒子光效（`SkinDef.fx`：攻击迸发 + 0.45s 待机微光）/钱币 `coins`/天赋 `talents`/道具 `gear` | `Player` | defs, hud, audio, status |
| [src/entities/animals.ts](src/entities/animals.ts) | 动物 AI 状态机（idle/wander/chase/windup/charge/flee/dying），行为差异全部由 `AnimalDef` 可选标志驱动：`boss`/`charge`(+`chargeSpeed/Dur/Min/Max`)/`poison`(蛇)/`charm`(妖狐,WASD 反向)/`retaliate`(山羊/海龟)/`flying`(海鸥/蝙蝠)/`marine`(鱼/龟/鲨,水域约束)/`latcher`+`meleeImmune`(吸血蝙蝠：latch 状态挂玩家头顶 `drainBlood`，吸满 20 点 `vanish`，近战与吸附中均不可被攻击)；灼烧 `burnT`、死亡时钱币掉落（`COIN_TABLE`+幸运天赋） | `Animal` | defs, audio |
...
| [src/core/](src/core) | `input.ts`（键鼠，`e.code` 体系）、`audio.ts`（合成音效单例 `sfx`）、`status.ts`（玩家状态效果统一管理 `Statuses`：poison/bleed/charm/drown——只管存在与时长并驱动 HUD 图标，伤害结算留在各来源处；新增状态在 `STATUS_INFO` 注册即可）、`save.ts`（localStorage，`SaveData` **v4**：货币/武器/皮肤/天赋/道具 gear/迷雾 explored（bit 打包 base64）/宝箱 openedChests；地图尺寸或世界生成变更时递增版本，旧版本档直接判废返回 null） | — | — |
...
- **不可被攻击的目标过滤有两处**：`Game.meleeStrike`（`meleeImmune`/`latched`）与 `Projectiles.update`（`latched`）——加新的免伤规则时两处都要改。
...
3. 行为级冒烟（浏览器控制台，依赖 `window.__game`）：移动后 `__game.player` 坐标变化；`__game.nodes` 找树 `meleeStrike` 后 hp 递减；篝火 `campfireAction('rest')` 后 localStorage 出现 `island-survival-save-v1`（version 4）。
