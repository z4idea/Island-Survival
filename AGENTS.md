...
- Last refreshed: 2026-06-13（refresh pass：神器祝福系统（天降光柱 + 狂暴守卫 + 祝福仪式）、神器武器（丘比特的弓/阿比努斯的权杖）、神器挂件（大天使的翅膀，圣翼冲刺提速 22 + `PLAYER_PHASE` 穿越静物）、坠入爱河 / 狂暴动物态落地后核对；玩家碰撞组改为 `Player.update` 末尾按 `colGroup` 集中切换、存档新增 relics、`Animal` 构造签名变更均已 grep 源码验证）
