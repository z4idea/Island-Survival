...
- Last refreshed: 2026-06-13（refresh pass：新增**驯养/伙伴系统** —— `Animal.companion`/`foe` + 独立 `updateCompanion`/`findEnemy`/`takeEnemyHit`/`fallAsCompanion`、敌人 AI 目标由写死 `p` 泛化为 `foe ?? p`（嘲讽转火）、AI 后段抽成共用 `finishUpdate`、`Game.companions`/`recruitCompanion`/`dismissCompanion`/`onCompanionDied`/`companionSpeedMul`/`restoreCompanions`、`combatTargets()` 排除伙伴、`SaveData.companions`、HUD `setCompanions`；前两轮的烹饪与小 Boss/战利品系统仍准确，复核猴子、神器、主循环、碰撞组、存档）
...
| [src/game.ts](src/game.ts) | **中枢**：Pixi/Rapier 初始化、主循环、战斗/采集结算、篝火与存档、商店动作、战争迷雾、洞穴、神器祝福、冥火、天降闪电、昼夜/天气/镜头/重生；**树上猴子**由 `buildNodes` 写入 `WNode.monkeyHidden/monkeyTriggered/monkeyTail`，`updateTreeMonkeys` 触发偷窃并生成 `Monkey`；`combatTargets()` 汇总普通动物与猴子供 5 类伤害源使用（**已排除 `companion`，避免误伤宠**）；**伙伴：`companions[]` + `recruitCompanion`/`dismissCompanion`/`onCompanionDied`/`companionSpeedMul`/`restoreCompanions`，收服在 `updateInteraction` 的 E 分支** | `Game`, `WNode` | 几乎全部模块 |
