...
- Last refreshed: 2026-06-14（refresh pass：仓库由**纯前端转为 monorepo**，新增 **Java 后端 `server/gateway/`**（Spring Boot 3.3.5 + Java 17 + Maven，账号注册/登录 JWT + 云存档读写，**联机改造第一步**）+ 根 `.gitignore`。后端见新增的 `## 后端 server/gateway` 段与对应调用链/守则/构建/验证条目。⚠️ **前端 `src/core/save.ts` 仍是 localStorage，尚未接入这套接口**——第一步的收尾待办。上一轮的世界事件/血月、伙伴/驯养、烹饪、小 Boss、猴子等前端系统全部复核仍准确）
- Refresh basis: 全量 `src/`、新增 `server/gateway/`（Java 源 + `pom.xml` + `application.yml`）、`.gitignore`、`package.json`、Vite/TypeScript 配置及近期提交；后端已本机静态编译 + 真实接口冒烟（注册→登录→存档 CRUD→401 鉴权）核对
- Confidence: high（前端 high；后端为本会话新建并冒烟全过，但前端尚未消费这些接口）

## Project Overview

类 Hades 俯视角**群岛**生存动作游戏。游戏客户端是**纯前端单页应用、无静态素材**：所有图形用 PixiJS Graphics/Text(emoji) 程序化绘制，音效用 WebAudio 合成，存档当前写 localStorage。⚠️ 仓库已转为 **monorepo**：`src/` 为前端游戏客户端，`server/gateway/` 为新增的 **Java 后端**（账号 + 云存档，联机改造第一步），但**前端尚未接入后端**——客户端仍走 localStorage，后端可独立编译/运行（详见 `## 后端 server/gateway`）。玩法主轴：采集/狩猎 → 钱币 → 商店（武器/天赋/皮肤/小木舟）→ 乘船或持海神三叉戟跨岛 → 击败随机岛屿山顶的 Boss 巨熊。生命恢复靠**进食**：生食（浆果/生肉）回血弱、生肉有概率食物中毒，需在篝火 `🍳 烹饪` 把生鲜烤成熟食（回血高、零风险、部分附带攻击/回血 buff）带在身上。每座非巨熊岛（含主岛）山顶另有**岛屿小 Boss**（精英化的虎/狼/蟹/蛇/猪），击杀掉**专属战利品**（永久被动），是可选的变强支线。用丘比特之弓把野兽射成「坠入爱河」后按 E 可**收服为伙伴**（掠食者作战宠、食草作加速光环，上限 2、会永久阵亡）。额外运行时事件包括：随机岛屿天降**神器祝福**光柱（被狂暴守卫环绕），按 E 举行仪式随机获得 5 件神器之一；约 8% 普通树藏有会偷资源/货币并逃跑的猴子。
...
Java 网关，与前端是**两套独立技术栈/构建体系**，靠目录边界隔离（`tsconfig.json` 仅 `include:["src"]`、Vite 只认自身配置，故前端工具链不会碰 `server/`）。职责：账号注册/登录（JWT）+ 云存档读写，**替代前端 localStorage**——但前端目前还没调它（见 Editing Guardrails）。
...
- **后端尚未与前端打通**：前端 `save.ts` 的 `loadSave/writeSave/clearSave` 仍是 localStorage。把它切到后端（加登录 UI + 改成调 `/api/auth/*` 与 `/api/save`、带 token）是「联机第一步」的收尾，**改动 `save.ts` 时注意这层迁移**，别假设后端已在使用中。
...
- **前端 `save.ts` 由 localStorage 切到后端接口**（联机第一步收尾）、或新增 `server/sim/` 等联机模拟服务
