...
- Last refreshed: 2026-06-14（refresh pass：仓库由**纯前端转为 monorepo**，新增 **Java 后端 `server/gateway/`**（Spring Boot 3.3.5 + Java 17 + Maven，账号注册/登录 JWT + 云存档读写，**联机改造第一步**）+ 根 `.gitignore`。后端见新增的 `## 后端 server/gateway` 段。**前端已接入后端**（同一会话收尾）：新增 [src/core/api.ts](src/core/api.ts) 客户端 + [src/core/save.ts](src/core/save.ts) 转**混合模式**（localStorage 仍为离线缓存，登录后叠加云同步）+ 标题画面登录/注册/退出 UI。上一轮的世界事件/血月、伙伴/驯养、烹饪、小 Boss、猴子等前端系统全部复核仍准确）
- Refresh basis: 全量 `src/`（含新增 `core/api.ts`）、`server/gateway/`（Java 源 + `pom.xml` + `application.yml`）、`.gitignore`、`package.json`、Vite/TypeScript 配置及近期提交；后端静态编译 + 接口冒烟 + 浏览器 E2E（注册→登录态→云存档 PUT/GET 往返→退出）核对
- Confidence: high（前后端均经端到端验证）

## Project Overview

类 Hades 俯视角**群岛**生存动作游戏。游戏客户端是**纯前端单页应用、无静态素材**：所有图形用 PixiJS Graphics/Text(emoji) 程序化绘制，音效用 WebAudio 合成，存档当前写 localStorage。仓库已转为 **monorepo**：`src/` 为前端游戏客户端，`server/gateway/` 为 **Java 后端**（账号 + 云存档，联机改造第一步，已上线）。**存档为混合模式**：localStorage 始终是离线缓存，登录账号后存档额外云端同步（见 `## 后端 server/gateway` 与存档链）。玩法主轴：采集/狩猎 → 钱币 → 商店（武器/天赋/皮肤/小木舟）→ 乘船或持海神三叉戟跨岛 → 击败随机岛屿山顶的 Boss 巨熊。生命恢复靠**进食**：生食（浆果/生肉）回血弱、生肉有概率食物中毒，需在篝火 `🍳 烹饪` 把生鲜烤成熟食（回血高、零风险、部分附带攻击/回血 buff）带在身上。每座非巨熊岛（含主岛）山顶另有**岛屿小 Boss**（精英化的虎/狼/蟹/蛇/猪），击杀掉**专属战利品**（永久被动），是可选的变强支线。用丘比特之弓把野兽射成「坠入爱河」后按 E 可**收服为伙伴**（掠食者作战宠、食草作加速光环，上限 2、会永久阵亡）。额外运行时事件包括：随机岛屿天降**神器祝福**光柱（被狂暴守卫环绕），按 E 举行仪式随机获得 5 件神器之一；约 8% 普通树藏有会偷资源/货币并逃跑的猴子。
...
> 本节描述**前端游戏客户端**（`src/`）的引擎不变量。后端 `server/gateway/` 是独立子系统，见 `## 后端 server/gateway`。
...
统一写入函数：`Game.saveNow()` → `writeSave`（[src/core/save.ts](src/core/save.ts)，key `island-survival-save-v1`，**version 4**）。调用点包括篝火休息、巨熊/小 Boss 击杀与伙伴阵亡；普通收服/遣散不立即写盘。**`writeSave` 先同步写 localStorage，再 fire-and-forget `pushCloudSave` 推云端（已登录时；失败不打断游戏）；`clearSave` 同步删云端**——游戏内同步存档路径不变，云同步是叠加层。**登录/读档协调**：标题画面登录成功 → `syncOnLogin`（[main.ts](src/main.ts)）`fetchCloudSave`，云端有则 `writeSaveLocal` 镜像回本地、云端空但本地有则把本地 `pushCloudSave` 种子上云；之后 `loadSave/hasSave` 照常只读 localStorage。字段含货币/武器库存（含神器武器）/武器等级/皮肤/天赋/道具 gear/**神器挂件 relics**/**随身熟食 food**/**小 Boss 战利品 trophies（player 内）+ 已杀小 Boss miniBossesDefeated（顶层）+ 伙伴物种 companions（顶层）**（均为可选字段，旧档缺省为空/缺省为零库存）/迷雾 explored（bit 打包 base64，`packExplored`/`unpackExplored`）/已开宝箱 `openedChests`（可选字段，旧 v4 档缺省为空）；洞穴水晶用 `removedNodes`（id ≥ 1_000_000）。**注意 `food`/`trophies`/`miniBossesDefeated`/`companions` 都是纯附加可选字段、不影响世界生成，故沿用 v4 不升版本（保住旧档）；只有改世界生成/地图尺寸/节点 id 语义才升版本。**小 Boss 布点由 isle/seed 决定且不消耗 rng，旧档世界一致。神器祝福光柱/守卫、树的 `monkeyTriggered` 状态和活动猴子都属于运行时事件，**不进存档**；读档会按相同 `seed+nodeId` 恢复哪些树藏猴子，但每棵树的触发状态重新开始。读档：`main.ts` `loadSave()`（版本 ≠4 直接判废返回 null）→ 传入 `Game.create`，用相同 `seed` 重生成世界后按 `removedNodes` id 列表跳过已采集节点 → `spawnAllAnimals` → `restoreCompanions(save.companions)` 在玩家附近满血归队。死亡 → `onPlayerDeath` → 死亡画面 → `Game.respawn()` 回到 `campfireId` 篝火、`clearStatuses()` 清全部状态并 `regenerateAnimals()`；重生同时清理活动猴子、重建普通动物和仍存活的伙伴。

## 后端 server/gateway（联机改造第一步，独立子系统）

Java 网关，与前端是**两套独立技术栈/构建体系**，靠目录边界隔离（`tsconfig.json` 仅 `include:["src"]`、Vite 只认自身配置，故前端工具链不会碰 `server/`）。职责：账号注册/登录（JWT）+ 云存档读写。**前端已接入**：[src/core/api.ts](src/core/api.ts) 是客户端，标题画面 [index.html](index.html)/[main.ts](src/main.ts) 提供登录/注册/退出 UI，登录后 `save.ts` 把存档镜像到云端（localStorage 仍作离线缓存，见存档链与 Editing Guardrails）。

- 技术栈：Spring Boot 3.3.5 + Java 17 + Maven；包根 `com.zhjj.islandsurvival`；端口 8080（可 `SERVER_PORT` 覆盖）。
- 数据库：144 上的 `mysql:8.0.45`，宿主端口 **13306**（映射容器 3306），库 `island_survival`，连接/密钥在 [server/gateway/src/main/resources/application.yml](server/gateway/src/main/resources/application.yml)（全部支持 env 覆盖：`DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD/JWT_SECRET/CORS_ORIGINS`）。`ddl-auto: update` 自动建表。

| 层 | 类 | 职责 |
| --- | --- | --- |
| 入口 | `GatewayApplication` | Spring Boot 启动类 |
| 控制器 | `controller/AuthController`（`/api/auth/register|login`）、`controller/SaveController`（`/api/save` GET/PUT/DELETE）| HTTP 端点 |
| 服务 | `service/AuthService`（BCrypt 校验 + 发 JWT）、`service/SaveService`（存档 upsert/读取/删除，抽 `version` 落列）| 业务逻辑 |
| 安全 | `security/JwtUtil`（HS256 签发/校验，subject=accountId）、`security/JwtInterceptor`（校验 `Bearer` 头、注入 `accountId` 到 request attribute）| 鉴权 |
| 实体 | `entity/Account`（username 唯一 + BCrypt `passwordHash`）、`entity/GameSave`（账号唯一、`data` LONGTEXT 整份 JSON、`version` 列）| JPA |
| 仓库 | `repository/AccountRepository`、`repository/GameSaveRepository` | Spring Data JPA |
| 配置 | `config/WebConfig`（CORS 放行前端 5188 + 把 `JwtInterceptor` **只挂在 `/api/save`**）| 横切 |
| 异常 | `exception/ApiException` + `GlobalExceptionHandler`（统一 `{error,message}` JSON）| 错误响应 |

**关键设计 —— 存档对后端是不透明 JSON blob**：整份 `SaveData` 原样存进 `game_save.data(LONGTEXT)`，后端**不解析其内部结构**，只用 Jackson 抽出顶层 `version` 落列（缺/非 int 则 400）。这刻意契合前端「纯附加可选字段、按 version 判废」策略：前端给 `SaveData` 加字段时**后端零改动**。一个账号一份存档（`account_id` 唯一，PUT 即 upsert）。

### 后端调用链

- **注册/登录**：`POST /api/auth/register|login` → `AuthController` → `AuthService`（register 查重 + `BCryptPasswordEncoder.encode`；login `matches` 校验）→ `AccountRepository` 存/查 → `JwtUtil.issue(accountId,username)` → 返回 `AuthResponse{token,accountId,username}`。
- **云存档**：`GET/PUT/DELETE /api/save`（带 `Authorization: Bearer <token>`）→ `JwtInterceptor.preHandle` 校验并把 `accountId` 写入 request attribute（失败抛 `ApiException` 401，被 `GlobalExceptionHandler` 转 JSON）→ `SaveController`（`@RequestAttribute accountId`）→ `SaveService`：GET 返回原始 JSON 文本或 204；PUT 抽 version 后 upsert；DELETE 按 accountId 删 → `GameSaveRepository`。
...
- 后端配置/密钥/连库：[server/gateway/src/main/resources/application.yml](server/gateway/src/main/resources/application.yml)（env 可覆盖）；依赖 [server/gateway/pom.xml](server/gateway/pom.xml)
- `.gitignore`（根，本轮新增）：忽略 `node_modules/`、`dist/`、`.codegraph/`、`server/**/target/`、IDE 与本地配置——`server/**/application-local.yml` 与 `*.env` 也忽略（放真实密钥用）
- `node_modules/`、`dist/`、`.codegraph/`、`server/gateway/target/`：生成物，勿入提交/勿手改
...
后端（`server/gateway/`，Maven）：

```powershell
# ⚠️ 关键坑：本机 mvn 默认 JAVA_HOME=D:\java1.8（Java 8），编不过 Spring Boot 3！
#    每次必须先把 JAVA_HOME 指到 Java 17（独立 21 未安装；GraalVM 21 仅在 IntelliJ .jdks 内）。
$env:JAVA_HOME='D:\Java\jdk'      # Java 17
mvn -f server/gateway/pom.xml -q -DskipTests clean compile   # 静态编译（验证底线）
mvn -f server/gateway/pom.xml -q -DskipTests spring-boot:run # 本地起服务，端口 8080
# PowerShell 后台启动用 Start-Process -WorkingDirectory '...\server\gateway'，
# 不要用 -f "带空格路径"（Start-Process 会在空格处截断）。
```

按用户规约：**前后端都只需通过静态编译即可**（前端 `npm run check`、后端 `mvn ... compile`），不要求每次跑完整 build。
...
- **后端：存档当不透明 JSON blob 存取，不要把 `SaveData` 字段拆成关系表/列**。前端 `SaveData` 演进时后端应零改动（只 `version` 落列），把存档结构知识保留在前端 [src/core/save.ts](src/core/save.ts)。新增需登录的接口要确认 `WebConfig` 的 `JwtInterceptor` 路径覆盖到它（当前只挂 `/api/save`，`/api/auth/**` 公开）。
- **前后端已打通（混合存档模式）**：`save.ts` 的 `writeSave` 始终先写 localStorage（同步、离线可用），登录后再 fire-and-forget 推云端；`loadSave/hasSave` 仍只读 localStorage，云端通过登录时的 `syncOnLogin`（[main.ts](src/main.ts)）镜像回本地后才被消费。**关键不变量：游戏内同步存档路径（`saveNow→writeSave`）没改、不阻塞主循环**——新增存档触点时沿用 `writeSave`/`clearSave` 即自动获得云同步，**不要**在游戏循环里 `await` 云请求。token 失效（401）时 api 层自动 `logout()`。云同步是 localStorage 之上的纯叠加层，未登录时静默跳过。
- **后端密钥不入库**：`application.yml` 里的默认 `JWT_SECRET`/`DB_PASSWORD` 仅供本地开发，生产用环境变量；真实密钥放 `application-local.yml` 或 `*.env`（已被 `.gitignore` 忽略）。
- **改后端模块/包结构、接口、数据库或部署**属于 Refresh Trigger（见下），且命中用户全局规约的 `AGENTS refresh trigger`。
...
- Vite/Node 约束、构建产物、部署方式变化
- **后端 `server/gateway/` 模块/包结构、`/api/*` 接口、JWT/鉴权、`SaveData` 存储映射、数据库或 `application.yml` 配置变化**
- **前端云同步/登录流程**（`api.ts`、`save.ts` 混合模式、`syncOnLogin`、标题画面账号 UI）变更，或新增 `server/sim/` 等联机模拟服务
...
11. 后端冒烟（`server/gateway/`）：`$env:JAVA_HOME='D:\Java\jdk'` 后 `mvn -f server/gateway/pom.xml -q -DskipTests clean compile` 零错误（静态编译底线）。需端到端时 `spring-boot:run` 起服务，按序打：`POST /api/auth/register{username,password}` 拿 token → `POST /api/auth/login` 复核 → 带 `Authorization: Bearer <token>` 的 `PUT /api/save`（body 为含 `version` 的 SaveData JSON）→ `GET /api/save` 应原样回传 → **无 token 的 `GET /api/save` 应 401** → `DELETE` 后再 `GET` 应 204。库侧（SSH 144：`docker exec mysql mysql -uroot -p123456 island_survival -e 'SHOW TABLES;'`）应见 `account`/`game_save` 两表、密码为 BCrypt `$2a$` 前缀。
12. 前端云同步 E2E（浏览器，需后端在跑 + 前端 origin 在后端 CORS 白名单内——默认 5188，autoPort 跑到别的端口时用 `$env:CORS_ORIGINS` 覆盖）：标题画面填用户名/密码点「注册」→ 账号区切到「☁️ 已登录：<用户名>」、`account-msg` 显示成功、`localStorage['island-survival-token']` 有值；带该 token `PUT/GET /api/save` 应往返一致；点「退出登录」后 token 清除、账号区回到登录态。未登录时游戏仍照常离线存档（localStorage）。
